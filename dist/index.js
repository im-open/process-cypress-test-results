  ...
  "src/utils.js"(exports2, module2) {
    ...
    async function readJsonResultsFromFile2(resultsFile2) {
      ...
      if (fs.existsSync(resultsFile2)) {
        ...
        if (!rawJson) {
          core2.info(`The results file '${resultsFile2}' does not contain any data.  No status check or PR comment will be created.`);
          return;
        }
        return JSON.parse(rawJson);
      } else {
        core2.setFailed(`The results file '${resultsFile2}' does not exist.  No status check or PR comment will be created.`);
        return;
      }
    }
...
var require_github2 = __commonJS({
  "src/github.js"(exports2, module2) {
    ...
    var markupPrefix = "<!-- im-open/process-cypress-test-results -->";
    async function createStatusCheck2(repoToken, markupData, conclusion, reportName2) {
      ...
      const git_sha = github.context.eventName === "pull_request" ? github.context.payload.pull_request.head.sha : github.context.sha;
      ...
      await octokit.paginate(octokit.rest.issues.listComments, {
        ...
        if (comments.length === 0) {
          ...
          if (existingComment) {
            ...
            core2.info("No comments were found.  A new comment will be created.");
          }
        }
      }).catch((error) => {
        core2.info(`Failed to list PR comments. Error code: ${error.message}.  A new comment will be created.`);
      });
...
${markupData}`,
          ...
          core2.info(`PR comment was updated.  ID: ${response.data.id}.`);
        }).catch((error) => {
          core2.setFailed(`An error occurred trying to update the PR comment: ${error.message}`);
        });
      } else {
    ...
    function format(dirtyDate, dirtyFormatStr, dirtyOptions) {
      ...
      if (matches) {
        ...
        formatStr = matches.reduce(function(result, token2) {
          return token2[0] === "'" ? result : result.replace(token2, "'" + _formatters.default[token2[0]](date, token2, null, options) + "'");
        }, formatStr);
      }
  ...
  <td>${data}</td>
    ...
    function getTestResultsMarkup(results, reportName2, truncatedMarkup = false) {
      ...
      if (!results || results.length === 0) {
        ...
        let failedTestSuites = results.map((r) => r.suites).flat().filter((s) => s && s.failures.length > 0);
...
async function run() {
  try {
    ...
    if (markupData.length > characterLimit) {
      core.info(`Truncating markup data due to character limit exceeded for github api.  Markup data length: ${markupData.length}/${characterLimit}`);
      ...
      if (markupData.length > 65535) {
        core.info(`Truncating markup data again due to character limit exceeded for github api. Markup data length: ${markupData.length}/${characterLimit}`);
        markupData = "Test outcome truncated due to character limit. See full report in output. <br/>" + markupData.substring(0, characterLimit - 100);
      }
    } else {
