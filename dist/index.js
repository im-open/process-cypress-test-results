    ...
    async function createPrComment2(repoToken, markupData) {
      try {
        if (github.context.eventName != 'pull_request') {
          core2.info(
            'This event was not triggered by a pull_request.  No comment will be created.'
          );
        }
        ...
        if (response.status !== 201) {
          ...
          core2.info(
            `Created PR comment: ${response.data.id} with response status ${response.status}`
          );
        }
      } catch (error) {
...
var resultsFile = core.getInput('results-file');
